...
PREFIX = {
    TARGET_APP: "datadog-plugin",
    TARGET_DEMO: "datadogPluginGradle",
    TARGET_GRADLE_PLUGIN: "datadogPluginGradle",
}
...
def parse_arguments(args: list) -> Namespace:
    ...
    parser.add_argument("-v", "--version", required=True, help="the version of the Datadog Gradle Plugin")
...
def github_create_pr(repository: str, branch_name: str, base_name: str, version: str, previous_version: str, gh_token: str) -> int:
    ...
    if previous_version:
        diff = "Updating Datadog Gradle Plugin from version {previous_version} to version {version}: [diff](https://github.com/DataDog/dd-sdk-android-gradle-plugin/compare/{previous_version}...{version})".format(previous_version=previous_version, version=version)
        body = "\\n".join([body, diff])
    data = '{"body": "' + body + '", ' \
           '"title": "Update Datadog Gradle Plugin to version ' + version + '", ' \
...
def git_push_changes(repo: Repo, version: str):
    ...
    repo.index.commit("Update Datadog Gradle Plugin to " + version)
...
def update_dependant(version: str, target: str, gh_token: str, dry_run: bool) -> int:
    branch_name = "update_datadog_gradle_plugin_" + version
